...
- D-30 **02.2 связана только индирекцией.** AC: каждый themeable-токен в `@theme` разрешается через `var()`, литералы запрещены (проверка — CI-тест); компоненты потребляют семантические токены, не сырые значения; обе темы AA. Ось скина и эстетика — свобода отдельной редизайн-сессии; D-20 НЕ пиннится. Обоснование: индирекция — предусловие любого выбора, а не выбор. Без AC редизайн законно выдаст DS v0.3 с литералами (как сейчас в `index.css:61-88`), и 02.6 упрётся в тот же блокер в середине волны, под жёстким порогом `pixel_skin_live`.
