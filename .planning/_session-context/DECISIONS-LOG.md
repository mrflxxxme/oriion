...
### 2026-07-15 | wave-2 contracts + W1-retro | product/founder-grill (grill-2026-07-15)

> Сессия: довести Wave 2 до состояния, в котором агенты исполняют фазы автономно и **корректно**. Вход — запрос founder'а «исчерпывающие спеки». Выход — D-29: исчерпывающие спеки НЕ пишутся (D-01 в силе), потому что корень не в формате спеки, а в трёх сквозных контрактах, которые врут агенту, и в том, что сигнал «готово» в Wave 1 врал систематически. Каждое решение — founder-verdict после анализа+рекомендации. Находки — из чтения канона и кода, не из памяти.

- D-29 **Гибрид (корневая развилка): D-01/ADR-040 D1 НЕ отменяется.** Сессия производит только то, что (а) не ротируется от 02.0/02.2 и (б) нужно более чем одной фазе: сквозные контракты + процессное ретро. Phase-local детали дорастают на discuss-шаге. Обоснование: 02.0 и 02.2 существуют ровно затем, чтобы изменить представление о 02.3–02.13 — исчерпывающая спека 02.13, написанная сегодня, пишется против допущений, которые обязаны быть опровергнуты.
- D-30 **02.2 связана только индирекцией; эстетика и ось — свобода редизайн-сессии.** ~~AC добавляется в 02.2~~ → **AMENDED в этой же сессии, до материализации:** новый AC НЕ нужен — требование уже стоит в каноне дважды. 02.1-retro несёт субстрат («var-индирекция radius/font/shadow», AC «переопределить через `[data-skin]`, проверено тестом»), 02.2 несёт материализацию (AC «var-индирекция 100%, **grep-гейт: ни одного литерального radius/font/shadow**» + «axe AA обе темы»), 02.6 назван отдельной осью поверх v0.3. Рекомендация была дана до прочтения спеки 02.2 — ошибка аналитика, исправлена без последствий.
  **Что из D-30 выживает и требует действия:** реальный конфликт — не «спека vs реальность», а **два контракта между собой**. `ui/design-tokens.md` §10.4 (source-of-truth по DS per ADR-028 DECISION-4) утверждает «структурные токены — spacing, radius, type scale — **не меняются**», тогда как D-20 и спека 02.2 требуют обратного. Агент, читающий DS-контракт, получает не тот ответ, что агент, читающий спеку фазы. §10.4 переписывается: гарантируется стабильность **имён/структуры** токенов (исходный интент — §10.4 сам пишет «mappings могут перенаправляться без touch every component»), значения варьируются по осям. D-20 не пиннится.
- D-31 **cost-budget v5: третий `cost_source: live_golden`**, конверт **НА ВОЛНУ** (не $/день — live-goldens это всплески по фазам, дневной кап структурно не той формы). Стартовый конверт Wave 2 = сумма seed-деклараций ~$25–60 (02.7 image-gen $20–50 доминирует). DoR п.8 переписывается: проверка против остатка конверта волны. Метр уже существует — `evidence.cost_usd`.
- D-32 **`user_production` капы (`per_request` / `per_organization_per_month` / `kill_switch`) = founder-зависимость, блокирующая 02.0**, отдельным RW-пунктом; без них фаза паркуется (ADR-040 D7). Обоснование: политика требовала задать их «перед wave-1-to-2 гейтом» — гейт подписан PASS 2026-07-10 с `null`. `null` молча означает «без потолка» при собственной оценке файла $5000–15000/мес.
- D-33 **Ретро = два разных артефакта.** 02.1-retro остаётся техническим hardening'ом (это не ретро, и в Wave 1 `01.1-retro` им тоже не был — слово занято дважды). Процессное ретро — выход ЭТОЙ сессии: `WAVE-1-RETRO.md`, где каждый режим отказа связан с конкретным изменением контракта. Ретро без изменения контракта — театр; D-30/31/32/34/35 и есть его содержание.
- D-34 **evidence-схема v2: поле `ac_ids[]` + машинная сверка.** `verify_evidence.py` требует, чтобы у каждого evidence-проверяемого AC было ≥1 PASS-доказательство, называющее его по id. Плюс: любой AC, утверждающий рантайм-поведение, обязан иметь **wiring-доказательство** — прогон реального пути (~$0.1), а не юнит. ADR-040 D11 сохраняется для дорогих golden'ов. Обоснование: схема v1 привязывает доказательство к ГЕЙТУ, а не к AC — AC-W1-4 утверждал «релей подключён», зелёные гейты доказывали другое утверждение, оба честны, связи нет. Мёртвый код проходит CI тривиально.
- D-35 **PII-скрин на входе `llm_gateway` → роутинг на RU-провайдера.** Переиспользует существующий `default_pii_detector` (сменный шов per 01.6/01.9b); попадание RU-ПДн → запрос идёт в YandexGPT/GigaChat, иначе допустим DeepSeek. Трансграничной передачи не происходит **по конструкции**. Приземляется в 02.1-retro — вынужденно, ДО 02.0. Обоснование: `grep -i 'dlp|screen|pii' backend/src/llm_gateway` → 0 совпадений. DLP охраняет границу коннекторов (`dlp_screen.py` = «outgoing-args connector exfiltration guard»), LLM-граница не охраняется ничем; «в LLM не уходят ПДн» было гипотезой о поведении пользователей, не свойством системы.
- D-36 **02.0 = закрытая дружеская бета, без артефакта согласия**; полный юрпакет (Политика/ТоС/согласие/РКН/юрист) гейтит **02.8**, не 02.0. Когерентно только вместе с D-35: согласие на трансграничку не нужно, если трансграничка не случается. Локализация БД в РФ (ст. 18.5) и трансграничная передача (ст. 12) — разные обязанности разных статей; хранение в РФ закрывает первую и не освобождает от второй. D-35 закрывает вторую архитектурно.
- Reversibility: D-29/33 reversible (догнать спеки можно позже). D-31/32/34 reversible (контрактные правки). **D-35 hard-to-reverse после материализации** — роутинг становится несущим для юр-позиции. **D-36 условно-reversible: отменяется автоматически, если D-35 не приземлится до 02.0** — тогда возвращается вопрос мини-согласия. OQ-03 (юрист) — OVERDUE с Wave 1, срок молча съезжал один раз; при D-36 привязан к 02.8 с владельцем.
