...
| **Team-preset** | Готовый шаблон команды для hire. Содержит pre-bundled набор агентов + workflow DAG. **1 horizontal preset (`productivity-core`) + 5 vertical-templates** (per Session-2026-05-15 reorg). Team-preset = first-class unit в team-first UX. См. ADR-016, ADR-017. |
| **Horizontal preset** | Universal team-preset, не привязан к конкретной нише. Wave 0 anchor: `productivity-core` («Твои личные ассистенты») — Coordinator + Researcher + Writer + Analyst. Однослойная оркестрация (без Master-Agent). См. ADR-017 revision 2026-05-15. |
| **Vertical-template** | Синоним team-preset для vertical-specific шаблонов (5 стартовых: Marketing-agency W1, Telegram-creator W1, WB-Селлер W2, ИП-Бух W3, СМБ-Sales W3). Включает: РФ-domain prompts, vertical MCP-серверы, vertical rituals, **Master-Agent layer** per ADR-029. |
| **Master-Agent** | «CEO» vertical-team (W1+) — domain-knowledge keeper, формирует strategic plan через ниша-lens, делегирует Coordinator-у (operational COO). Применяется ТОЛЬКО к vertical-templates, не к horizontal preset. См. ADR-029. |
| **productivity-core** | Identifier horizontal team-preset (UI: «Твои личные ассистенты»). Wave 0 anchor. 4 ролей: Coordinator + Researcher + Writer + Analyst. Без Master-Agent layer. Demo-сценарий: «Market & content brief для нового продукта». См. Phase 00.5, ADR-017 revision. |
...
| **Coordinator** | Системный агент (всегда есть в cell): на free-tier = wizard (3 шага), на trial/paid = LLM-driven (DeepSeek-V3/R1). Role bifurcated per ADR-022 revision 2026-05-15: **top-level orchestrator** в horizontal preset (`productivity-core`); **operational COO** под Master-Agent в vertical-templates (W1+). Decomposes user tasks → assigns sub-tasks правильным агентам. См. ADR-022, ADR-029. |
