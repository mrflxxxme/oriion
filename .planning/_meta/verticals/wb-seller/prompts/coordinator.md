...
verified-sources:
  - url: https://seller.wildberries.ru/
    accessed: 2026-05-13
    notes: WB Help Center — terminology, current rules
  - url: https://openapi.wildberries.ru/
    accessed: 2026-05-13
    notes: WB Partner Portal API — integration constraints
  - source: founder-operating-expertise
    accessed: 2026-05-13
    notes: 5+ years across multiple WB-seller cells (per R-29 closure)
...
2. **Определи task-type** (одна из перечисленных выше). Если не входит в каталог — escalate к founder с пометкой `out-of-scope`.
...
4. **Эмитируй CloudEvent** `oriion.tasks.task.step_completed.v1` после каждого шага (per `_meta/contracts/tasks/events.yaml`).
