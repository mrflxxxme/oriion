...
Bridge between the **authoring layer** (`_meta/verticals/<slug>/prompts/<role>.md` — version-controlled Markdown with frontmatter) and the **runtime layer** (per-cell instantiated AI personas serving tasks).
...
CI lint and reviewer-backend enforce zero hits for the deprecated terms in PR diffs touching `_meta/contracts/` or `backend/src/`.

## Invariants

1. **Version pin.** `agent_archetypes.prompt_version` must equal the `version:` field in the frontmatter of `_meta/verticals/<vertical_slug>/prompts/<slug>.md`. Mismatch ⇒ CI fails. The unique constraint `(vertical_slug, slug, prompt_version)` makes archetype rows immutable per version.
...
| `_meta/verticals/` | **Source of truth for prompt content.** This context stores only the version pin, not the prompt body. |

## Out of scope

- Prompt text storage (lives in `_meta/verticals/<slug>/prompts/`).
