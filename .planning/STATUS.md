...
**Phase 00.5b (runtime, next session)** — ⏳ **Pending**. Per Topic 2 cut-list (chunked deliverable):
* Commit 2: Router wiring + provider DI + exception handlers (main.py overhaul + llm_gateway/deps.py + lifespan provider boot)
* Commit 3: Per-module coverage gates uniform (F-P5-6) + router-test convention pick + doc (F-P5-5)
* Commit 4: `LLMGatewayModel` Pydantic-AI Model ABC subclass + `pydantic_ai_test_model` fixture with canned `(role_key, scenario_id)` keyed responses (T3 resolution)
* Commit 5: `agents` bounded context + 4 Pydantic-AI agents (Coordinator + Researcher + Writer + Analyst) + `delegate_task` tool + role_prompt_loader + first-pass alignment hardening of 4 role-prompts (T5 resolution)
* Commit 6: `tasks` context + `runtime` (orchestrator + SSE publisher + budget guard); BudgetExceeded test (F-P5-2)
* Commit 7: Demo flow integration test via `pydantic_ai_test_model` (T4 hybrid) + 3 chat_stream SSE provider tests (F-P5-4 partial: DeepSeek + Yandex + GigaChat) + runnable `scripts/demo_market_brief.py` for Phase 00.6 staging gate-evidence collection
* Commit 8: ADR-024 amendment for sanctioned `llm_gateway → billing.models` (E2 — deferred from Phase 00.5a because router wiring touches the import surface) + Exit ritual + Phase 00.5 ✅ Complete flip
