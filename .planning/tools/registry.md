...
Любой `.claude/agents/<role>/tools-allowlist.md` ИЛИ `verticals/<slug>/prompts/<role>.md`
...
Mapped из `.planning/contracts/<context>/api.yaml` operationIds. Каждый slug —
canonical name для cross-context referencing. Backend implementations expose эти endpoints
per OpenAPI spec.

### 2.1 Tasks context (`contracts/tasks/api.yaml`)
...
### 2.2 LLM-gateway context (`contracts/llm-gateway/api.yaml`)
...
### 2.3 IAM context (`contracts/iam/api.yaml`)
...
### 2.4 Multitenancy context (`contracts/multitenancy/api.yaml`)
...
### 2.5 Agents context (`contracts/agents/api.yaml`)
...
| `Write` | File create | excluding `contracts/*` (architect-only escalation) and `.claude/agents/_shared/*` (architect/memory-curator-only) |
...
- `contracts/*/api.yaml` — REST operationIds (mapped в category 2)
- `.claude/agents/*/tools-allowlist.md` — per-role granted slugs
- `verticals/<slug>/prompts/*.md` `tools_allowed:` frontmatter — must reference registry slugs
