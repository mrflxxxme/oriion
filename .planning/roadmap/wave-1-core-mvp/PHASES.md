...
| 01.4-ui | Memory UI — «Что помнит [агент]» view/edit/delete panel (grill Q6) | ✅ Merged | [ADR-011](../../decisions/ADR-011-memory-2-level.md) | — |
...
| 01.8b | Auth OAuth — Yandex ID + VK ID flow + account-linking | ⛔ Descoped (auth email-only; RW-02 снята 2026-07-09; вернуть = новым решением) | [ADR-007](../../decisions/ADR-007-authentik-then-keycloak.md) | — |
| 01.8c | **Autonomy hardening** (сервисная, ADR-040): роли → нативные сабагенты + OpenAPI-snapshot растяжка + docs-freshness CI + brand-ренейм в коде («Профики», #111) | ✅ Merged | [ADR-040](../../decisions/ADR-040-execution-spec-contract.md), [seed-spec](./phases/01.8c-autonomy-hardening.md) | — |
| 01.9a | DLP activation — context-aware INN-детектор (FP 11%→0%, golden-корпус 720 строк), оба security-флага ON в проде (ADR-040 D10; закрыты DV-04/DV-05) | ✅ Merged | [ADR-039](../../decisions/ADR-039-security-guardrails-context.md), [seed-spec](./phases/01.9-mcp-servers.md) | — |
| 01.9b | Коннекторы read+draft — 3 native-tool коннектора (telegram-bot Bot-API / yandex-disk / imap-smtp) + capability-gate (DANGEROUS-send deny до approval-UI) + KMS creds-store `mcp.connector_credentials` + DLP-скрин исходящих аргументов | ✅ Merged | [ADR-041](../../decisions/ADR-041-connector-architecture-native-tools.md), [ADR-013](../../decisions/ADR-013-mcp-protocol.md), [seed-spec](./phases/01.9-mcp-servers.md) | — |
| 01.10 | Telegram-крейтор 2-й vertical (Master + prompts + Bot-API content-демо) + horizontal prompt-hardening. **Process (grill 2026-07-03):** research-first фаза (domain-brief: ЦА + поведенческий паттерн + ключевые аспекты, cited) → autonomous draft-промпты grounded в brief → golden+evaluator → founder review-gate `draft→reviewed` (brief едет в PR). Amend ADR-026 | ✅ Merged | [ADR-029](../../decisions/ADR-029-master-agent-vertical-templates.md), [ADR-017](../../decisions/ADR-017-vertical-templates.md), [ADR-026](../../decisions/ADR-026-vertical-expertise-pipeline.md), [seed-spec](./phases/01.10-telegram-creator-vertical.md) | [RW-03](../../FOUNDER-RUNWAY.md) гейтит ТОЛЬКО live-демо; dev-часть автономна |
...
| 01.12 | Dashboard UI + Onboarding wizard (3 шага) + trial-cell auto-spawn + horizontal-vs-vertical routing + demo-сценарии | ✅ Merged | [ADR-001](../../decisions/ADR-001-modular-monolith.md), [ADR-022](../../decisions/ADR-022-coordinator-wizard-llm-hybrid.md), [ADR-016](../../decisions/ADR-016-team-first-ux.md), [seed-spec](./phases/01.12-dashboard-onboarding.md) | integration last |

## Автономная очередь + закрытие волны (ADR-040 D4, интервью 2026-07-07)

- **Очередь (плановая):** `01.8c → 01.9 → 01.4-ui → 01.10 → 01.12`.
- **Must для закрытия Wave 1:** 01.9 + 01.4-ui + 01.10 + 01.12 (+ уже смерженные).
- **Фактический порядок исполнения (product-first, 2026-07-08/09):** `01.4-ui → 01.9a → 01.9b → 01.10 → 01.12 → 01.8c` — продуктовые must-фазы вперёд, сервисная 01.8c последней.
- **Parked (🅿️):** 01.3b ([RW-04](../../FOUNDER-RUNWAY.md)) · 01.11 (RW-05) — вливаются в очередь по разблокировке RUNWAY; НЕ блокируют закрытие волны, при гейте переносимы в Wave 2. **01.8b (OAuth) — ⛔ descoped** (auth email-only, RW-02 снята).
...
Нормативный гейт — [gates/wave-1-to-2.md](../../gates/wave-1-to-2.md) (**подписан PASS 2026-07-10**, Wave 1 формально закрыта). Секция «Метрика успеха» в [README.md](./README.md) — до-ревизионная (2026-05), см. ревизионный блок там.
