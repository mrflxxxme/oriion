...
Этот файл — единственный source-of-truth для того, как AI-агент работает в проекте Oriion (рабочее имя per ADR-040 D3; в старых доках/коде встречается прежнее TEAMLY_RU — это тот же проект). Здесь только «как работать», не «что за проект» (это в [`../README.md`](../README.md)).
