...
**Каждый factual claim** в payload содержит `source_idx` → `sources[idx]`. Без source — claim invalid (anti-hallucination Level B per [DECISION-11](../../../decisions/ADR-028-policies-registry.md#decision-11)).
...
4. **Adversarial probes 100% pass-rate** — gate перед promotion `draft` → `reviewed`. См. `verticals/wb-seller/golden-dataset/adversarial/`.
