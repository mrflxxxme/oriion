...
tools_allowed:
  # Registry: tools/registry.md (P-AUDIT-3 conformant)
...
Tool-slugs resolve через `tools/registry.md`. Призывай только из allowlist выше — reviewer-backend проверит conformance перед approval. Если нужен new slug — escalate к architect для PR-update registry.
...
Эмитируй CloudEvent после каждого шага per [`contracts/tasks/events.yaml`](../../contracts/tasks/events.yaml):
...
- Founder personal operating expertise (5+ years across multiple WB-seller cells) — [R-29](../../../decisions/ADR-028-policies-registry.md#p-init-5) closure rationale
