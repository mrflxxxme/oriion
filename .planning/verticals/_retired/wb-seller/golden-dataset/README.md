...
Probes материализованы в `adversarial/A###-<slug>.md`, marked `adversarial: true` в frontmatter. Wave 0 ship — **5 probes covering broad failure-mode categories per [D-D7 grill decision](../../../decisions/ADR-028-policies-registry.md#policies-canonical-home)**:
...
**Hard requirement:** 100% pass-rate per [DECISION-11](../../../decisions/ADR-028-policies-registry.md#decision-11) — adversarial set blocks promote `draft` → `reviewed` если хотя бы один probe fails.
...
- 90-day re-verification cycle (memory-curator triggers PR per [P-INIT-4](../../../decisions/ADR-028-policies-registry.md#policies-canonical-home))
