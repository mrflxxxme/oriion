...
**Wave 1 (Core MVP) — ✅ ЗАКРЫТА (2026-07-10, gate [`wave-1-to-2`](.planning/gates/wave-1-to-2.md) = PASS).** Все три вычислимых порога MET (AC pass-rate ~1.0 · must-фазы merged · DEFERRED-VERIFICATION без открытых P1). Смержено: 01.1-retro … 01.8 + **01.4-ui memory panel · 01.9a DLP activation · 01.9b connectors · 01.10 telegram_creator vertical · 01.12 dashboard+onboarding** (must-множество) + **01.8c** (нативные сабагенты D8 · OpenAPI-snapshot+drift-CI D2 · docs-freshness CI D9 · JOURNAL-архив D12; PR #109/#111/#112) + **brand-rename** teamly→«Профики» (`oriion` — внутренний codename). Обе вертикали (`telegram_creator` + `agency_marketing_ru`) — `reviewed`. `main` HEALTHY; задеплоено + VPS-verified. Итоговая справка → [`WAVE-1-SUMMARY`](.planning/roadmap/wave-1-core-mvp/WAVE-1-SUMMARY.md). **Next → планирование Wave 2 (отдельная сессия):** 02.1-retro + 02.0 friend-validation. Parked → Wave 2 (нужны креды): 01.3b ЮKassa (RW-04) · 01.11 Telegram Business (RW-05); **01.8b OAuth — descoped** (auth email-only, RW-02 снята). Состояние — [`.planning/STATUS.md`](.planning/STATUS.md); разблокировки — [`.planning/FOUNDER-RUNWAY.md`](.planning/FOUNDER-RUNWAY.md).
