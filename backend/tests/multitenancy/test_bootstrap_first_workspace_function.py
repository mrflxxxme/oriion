...
async def test_inserts_workspace_cell_cell_member_under_oriion_app_role(
    ...
    # to oriion_app for the bootstrap call. Use a fresh connection so the
    # SET LOCAL ROLE is bounded to this test's TX.
    ...
    async with db_engine.connect() as conn:
        async with conn.begin():
            ...
            cell_row = await session.execute(
                text(
                    "SELECT slug, display_name FROM multitenancy.cells WHERE id = :c"
                ),
                {"c": cell_id},
            )
            cr = cell_row.first()
