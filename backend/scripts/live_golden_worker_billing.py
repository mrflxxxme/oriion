...
The per-day cap (R-04) is proven by the focused `live_cap_check.py` against a
freshly-started worker. NOTE: in the local-Windows setup a worker reliably
processes ONE dispatch task per process; a 2nd dispatched task stalls before
`task.started` (a separate worker-infra finding, distinct from the redis
per-message fix that enabled Redis-SSE above — see HANDOFF/JOURNAL). So each
live golden runs against its own fresh worker.
...
async def main() -> int:
    ...
    async with httpx.AsyncClient(base_url=BASE, timeout=30.0) as c:
        ...
        print(
            f"    status={status} cost={cost} in_tok={task.get('total_input_tokens')} "
            f"out_tok={task.get('total_output_tokens')}"
        )
        _gate(
    ...
    print(
        "GATE (worker-path + Redis-SSE + RLS-billing + 01.3 trial/debit): "
