...
Run against a FRESHLY-restarted worker so the capped task is the worker's first
message (the worker's per-message asyncio.run() reuses process-global async
resources on the 2nd+ message — a separate worker-infra bug; this isolates the
cap assertion from it).
