...
6. **Exit ritual** — JOURNAL append + HANDOFF rewrite (house rule; review-gate blocks merge without it) **+ doc-sync (ADR-040 D9):** (a) README «текущая фаза» actual; (b) runbook for the touched feature updated/created under `docs/runbooks/`; (c) the merged phase's spec-file Status is not Pending; (d) if the phase reorganized the roadmap → the affected `gates/*.md` are synced in the same PR (D5); **(e) phase state-summary — founder directive 2026-07-09 (continuous): emit a «Состояние проекта» справка for EVERY phase — two blocks: (1) техническое состояние (что изменилось: модули / гейты / deploy / схема / флаги) + (2) реализованные пользовательские сценарии (что теперь может конечный пользователь end-to-end — дельта фазы). Append it to the phase JOURNAL entry AND prepend the rolling [`.planning/PROJECT-STATE.md`](../../.planning/PROJECT-STATE.md), and put it in the founder-facing PR body / run report. Template: [`.planning/_meta/state-summary-template.md`](../../.planning/_meta/state-summary-template.md).**
...
At run end estimate spend (phases × avg task cost vs `cost-budget.yaml` dev_team caps) and include it in the run's `complete` RUN-QUEUE entry. Judge-panels and heal fix-loops count toward the same per-day budget; degrade panel N=3→2 when tight (see judge-panel.md). Per-day caps: **soft $50 / hard $75** (v4, founder 2026-07-09; internal Claude-agent-team budget, NOT live-LLM). Soft-cap behavior: finish the current phase cleanly, then park the queue + notify — never abandon a phase mid-flight for the soft cap. NB: раннер меряет spend приблизительно (нет live-метра токенов Claude Code) — это порог-ориентир.
...
**On WAVE close (founder directive 2026-07-09)** — when all must-phases of a wave are merged + the wave-gate's autonomously-computable thresholds are met: additionally emit a **wave state-summary** doc `roadmap/<wave>/WAVE-N-SUMMARY.md` — полное техническое состояние волны (архитектура / модули / гейты / deploy) + **все реализованные пользовательские сценарии** (что конечный пользователь может пройти end-to-end) + что нужно от founder для формального закрытия — and present it to the founder. It is the input to the founder's wave-gate sign-off. Template: [`.planning/_meta/state-summary-template.md`](../../.planning/_meta/state-summary-template.md).
