...
7. **Tools allowlist per role — two layers.** Каждая роль имеет `tools-allowlist.md`. **Coarse layer (harness-enforced):** спавнабельный `agents/<role>.md` объявляет `tools:` — Claude Code реально ограничивает роль этим набором. **Fine layer (capability-enforced через PreToolUse-хук):** path/sub-command-скоуп read-only/gate-ролей (reviewer-* пишут ТОЛЬКО в `revisions/`, verifier — в `verification-reports/`, architect — в `decisions/`/`_meta/audits/`/`risks/`, evaluator — в `.tmp/evaluator-runs/`/`evidence/`; Bash-мутации git-commit/push/reset/install/rm-rf/… запрещены) форсит **`scripts/autonomy/role_scope_hook.py`** (chip task_dd666049, закрыл 01.8c SECURE-P2): читает PreToolUse-payload `agent_type` (fail-open для main-агента/имплементеров, fail-closed на Write-скоупе рестрикт-роли). **FOUNDER-ACTION: armed** через `.claude/autonomy/settings.hook-snippet.json` (Claude не ставит хуки сам) — runbook `docs/runbooks/role-scope-hook.md`. `check_subagents.py` проверяет наличие/конформность spawn-entry + валидность tool-имён + что scoped-роли существуют (НЕ прозу-скоуп — им владеет хук). Попытка выйти за allowlist = блок хуком (exit 2) → роль эмитит verdict + делегирует мутацию имплементеру.
